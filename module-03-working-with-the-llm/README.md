...
> The agent loop is running, but it has no voice. This module gives the Pathfinder AI a front door — a CLI chat interface where crew members type questions and see answers stream back token by token, an HTTP API so the bridge console and mobile devices can connect, and session storage so conversations survive between watches. By the end of this module the ship has a working chatbot.
...
The model sees the full context every time — that is how it "remembers" the conversation. The catch is that the history grows with every turn. When it exceeds the model's context window you need to truncate or summarise (covered in Module 8).
