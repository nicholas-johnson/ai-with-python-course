...
[Chat API — FastAPI + SSE streaming]           (Module 1)
    ↓
[Router Agent — classifies and delegates]      (Module 6)
    ├── [RAG Agent — searches documents]       (Modules 5, 9)
    ├── [Tool Agent — calls MCP tools]         (Modules 1, 4)
    └── [Analyst Agent — structured facts]     (Module 8)
    ↓
[Supervisor — synthesises, critiques]          (Module 6)
    ↓
...
**Router** — analyses the query and decides which specialist(s) to invoke (Module 6). A navigation question goes to the tool agent with sensor tools. A research question goes to the RAG agent. A complex question may fan out to multiple agents.

**RAG agent** — retrieves relevant documents from the vector store, applies adaptive retrieval routing (Module 9), and builds a grounded prompt with citations (Module 5).
...
**Analyst agent** — extracts structured facts from tool results and RAG passages (Module 8). Builds or queries a knowledge graph for relationship questions.

**Supervisor** — orchestrates the workflow, runs critique loops, and assembles the final answer (Module 6). If the first answer is low-confidence, it triggers a revision.
...
**Production hardening** — trace IDs propagate through every step, retries handle transient failures, cost tracking enforces budgets (Module 10).
