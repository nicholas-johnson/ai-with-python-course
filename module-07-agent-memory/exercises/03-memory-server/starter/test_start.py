"""Tests for Exercise 03 — Memory MCP Server."""
...
def _import_server():
    """Import server.py."""
    mod = _import_module("server")
    if mod and hasattr(mod, "mcp"):
        return mod
    raise FileNotFoundError("server.py not found or missing mcp attribute")


def _import_client():
    """Import start.py."""
    mod = _import_module("start")
    if mod:
        return mod
    raise FileNotFoundError("start.py not found")
