...
Crew member
    ↓
[Chat API — FastAPI + SSE streaming]           (Module 3)
    ↓
[Router Agent — classifies and delegates]      (Module 7)
    ├── [RAG Agent — searches documents]       (Modules 6, 10)
    ├── [Tool Agent — calls MCP tools]         (Modules 2, 5)
    └── [Analyst Agent — structured facts]     (Module 9)
    ↓
[Supervisor — synthesises, critiques]          (Module 7)
    ↓
[Guardrails — validates output]               (Module 4)
    ↓
[Session memory — stores conversation]         (Module 8)
    ↓
...
**Chat entrypoint** — a FastAPI app with SSE streaming (Module 3). The crew member sends a message; tokens stream back in real time. Tool calls appear as events so the user sees what the AI is doing.

**Router** — analyses the query and decides which specialist(s) to invoke (Module 7). A navigation question goes to the tool agent with sensor tools. A research question goes to the RAG agent. A complex question may fan out to multiple agents.

**RAG agent** — retrieves relevant documents from the vector store, applies adaptive retrieval routing (Module 10), and builds a grounded prompt with citations (Module 6).

**Tool agent** — discovers and calls MCP tools (Module 5). Sensor reads, crew lookups, and log searches are all available. Tool calls are validated against schemas and gated by auth scopes.

**Analyst agent** — extracts structured facts from tool results and RAG passages (Module 9). Builds or queries a knowledge graph for relationship questions.

**Supervisor** — orchestrates the workflow, runs critique loops, and assembles the final answer (Module 7). If the first answer is low-confidence, it triggers a revision.

**Guardrails** — validates the final response against schema, content filters, and confidence thresholds (Module 4). Rejected responses trigger a fallback.

**Memory** — session memory persists the conversation; long-term memory stores crew preferences (Module 8).
...
- **New memory backends** — implement the `SessionBackend` Protocol from Module 3. Swap in Redis, Postgres, or any other store.
